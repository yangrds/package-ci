<template>
    <div class="test-details">
        <div class="test-task-scroll">
            ...
            <div class="task-info" v-load="loading" element-loading-background="rgba(0, 0, 0, 0.8)">
                <div class="write-dialog">
                    <div class="write-title">
                        <span>项目部署</span>
                    </div>
                    ...
                    <el-col :span="6" class="write-col">
                        <div class="write-child" style="padding-left: 90px;">
                    ...
                    <el-col :span="6" class="write-col">
                        <div class="write-child" style="padding-left: 90px;">
                            <span class="label" style="width: 80px;">QA签署时间</span>
                            <div class="text">
                                <span>{{ details.QASignDate ? dateFormat('YY-mm-dd HH:MM:SS', details.QASignDate) :
                                        '--'
                                }}</span>
                            </div>
                        </div>
                    ...
                    <el-col :span="6" class="write-col">
                        <div class="write-child" style="padding-left: 90px;">
                            <span class="label" style="width: 80px;">PM签署时间</span>
                            <div class="text">
                                <span>{{ details.PMSignDate ? dateFormat('YY-mm-dd HH:MM:SS', details.PMSignDate) :
                                        '--'
                                }}</span>
                            </div>
                        </div>
                    ...
                    <el-col :span="6" class="write-col">
                        <div class="write-child" style="padding-left: 90px;">
                            <span class="label" style="width: 80px;">QA签署状态</span>
                            <div class="text">
                                <span>{{ statusText(details.QASignStatus).text }}</span>
                            </div>
                        </div>
                    </el-col>
                    ...
                    <el-col :span="6" class="write-col">
                        <div class="write-child" style="padding-left: 90px;">
                            <span class="label" style="width: 80px;">PM签署状态</span>
                            <div class="text">
                                <span>{{ statusText(details.PMSignStatus).text }}</span>
                            </div>
                        </div>
                    </el-col>
                    ...
                    <el-col :span="24" class="write-col" v-if="details.status === '0'">
                        <div class="write-child" style="padding-left: 90px;">
                            ...
                            <div class="text">
                                <el-popconfirm @confirm="vm_task_audit(1, 'QA')" @cancel="vm_task_audit(2, 'QA')"
                                    title="您正在签署该版本生产上线流程" confirmButtonText="同意" cancelButtonText="拒绝">
                                    <template #reference>
                                        <el-button :disabled="!!details.QASignStatus">
                                            {{ `QA${details.QASignStatus === 0 ? '签署' :
                                                    statusText(details.QASignStatus).text
                                                    }`
                                            }}</el-button>
                                    </template>
                                </el-popconfirm>

                                <el-popconfirm @confirm="vm_task_audit(1, 'PM')" @cancel="vm_task_audit(2, 'PM')"
                                    title="您正在签署该版本生产上线流程" confirmButtonText="同意" cancelButtonText="拒绝">
                                    <template #reference>
                                        <el-button :disabled="!!details.PMSignStatus">
                                            {{ `PM${details.PMSignStatus === 0 ? '签署' :
                                                    statusText(details.PMSignStatus).text
                                                    }`
                                            }}</el-button>
                                    </template>
                                </el-popconfirm>

                                <el-button v-if="details.releaseStatus != '1'"
                                    :disabled="(details.PMSignStatus != 1 || details.QASignStatus != 1)" type="warning"
                                    @click="vm_prod_create">
                                    项目部署</el-button>
                            </div>
                        </div>
                    </el-col>
...
import { Members_read, test_create, test_release, task_update, uat_task, uat_release, prod_task, prod_release } from '@/http/api'
...
async function vm_task_audit(status: number, type: string) {
    ...
    if (type === 'QA') {
        ...
        params.QASignStatus = status
        params.QASignDate = Math.round(new Date() as any)
        params.QASignReason = details.reason
    } else if (type === 'PM') {
        if (!counter.permission(details.project, ['pm'])) {
            ElMessage.warning('权限不足')
            return
        }
        params.PMSignStatus = status
        params.PMSignDate = Math.round(new Date() as any)
    } else {
        return
    }
...
async function vm_prod_create() {
    if (!counter.permission(details.project, ['dev'])) {
        ElMessage.warning('权限不足')
        return
    }
    ElMessageBox.confirm(
        ...
        .then(async () => {
            loading.value = true
            const releaseRes = await prod_release({
            ...
            if (releaseRes.code === 200) {
                ...
                if (updateRes.code === 200) {
                    ...
                    ElMessage.error(`【部署成功】，但是工单数据更新失败：${releaseRes.msg}`)
                }
            } else {
                ElMessage.error(`【部署失败】${releaseRes.msg}`)
            }
...
async function vm_prod_task() {
    const res = await prod_task({ task_id: route.params.id })
    if (res.code === 200) {
        let obj = res.data || {}
