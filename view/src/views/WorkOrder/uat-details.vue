<template>
    <div class="test-details">
        <div class="test-task-scroll">
                ...
                <div class="write-dialog">
                    ...
                    <el-col :span="24" class="write-col">
                        <div class="write-child">
                            ...
                            <div class="text">
                                <span>{{ details.project ? getdevs(details.project?.devs) : '--' }}</span>
                            </div>
                        </div>
                    </el-col>
                </div>
            ...
            <div class="task-info" v-load="loading">
                <div class="write-dialog">
                    <div class="write-title">
                        <span>项目部署</span>
                    </div>
                    ...
                    <el-col :span="24" class="write-col" v-if="details.status === '0'">
                        <div class="write-child" style="padding-left: 80px;">
                            ...
                            <div class="text">
                                <el-button v-if="details.releaseStatus === '0'" type="warning" @click="vm_uat_create">
                                    部署版本
                                </el-button>
                                <el-button v-if="details.status === '0'" @click="vm_task_audit('1')">UAT验收</el-button>
                                <el-button v-if="details.status === '0'" @click="vm_task_audit('-1')">UAT退回</el-button>
                            </div>
                        </div>
                    </el-col>
                </div>
...
import { Members_read, test_create, test_release, task_update, uat_task, uat_release } from '@/http/api'
...
async function vm_uat_create() {
    ...
    ElMessageBox.confirm(
        ...
        .then(async () => {
            loading.value = true
            const releaseRes = await uat_release({
...
async function vm_uat_task() {
    const res = await uat_task({ task_id: route.params.id })
    if (res.code === 200) {
        let obj = res.data || {}
